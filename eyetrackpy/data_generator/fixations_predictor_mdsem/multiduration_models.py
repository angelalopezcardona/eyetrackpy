...
import sys
import os
from keras.layers import Layer, Input, Multiply, Dropout, TimeDistributed, LSTM, Activation, Lambda, Conv2D, Conv3D, Dense, GlobalAveragePooling2D, MaxPooling2D, ZeroPadding2D, UpSampling2D, BatchNormalization, Concatenate
...
import scipy.io
import math
from eyetrackpy.data_generator.fixations_predictor_mdsem.attentive_convlstm_new import AttentiveConvLSTM2D
from eyetrackpy.data_generator.fixations_predictor_mdsem.dcn_resnet_new import dcn_resnet
from eyetrackpy.data_generator.fixations_predictor_mdsem.gaussian_prior_new import LearningPrior
from eyetrackpy.data_generator.fixations_predictor_mdsem.sal_imp_utilities import *
from eyetrackpy.data_generator.fixations_predictor_mdsem.xception_custom import Xception_wrapper
...
def sam_resnet_3d(input_shape = (224, 224, 3),
    ...
    # Attentive ConvLSTM
    att_convlstm = Lambda(lambda x: K.repeat_elements(K.expand_dims(conv_feat,axis=1), nb_timestep, axis=1),
    ...
    outs_final = TimeDistributed(UpSampling2D(size=(ups,ups), interpolation='bilinear'))(x)
...
def xception_3d(input_shape = (224, 224, 3),
           filt_3d = 128,
           conv_filters=128,
           lstm_filters=128,
           att_filters=128,
    ...
    ### ENCODER ###
    xception = Xception_wrapper(include_top=False, weights='imagenet', input_tensor=inp, pooling=None)
    print('xception output shapes:',xception.output.shape)
    if freeze_enc:
        for layer in xception.layers:
	        layer.trainable = False
    ...
    print('att_convlstm output shape',att_convlstm.shape)
...
def xception_3stream(
                    input_shape = (shape_r, shape_c, 3),
                    conv_filters=512,
                    verbose=True,
                    print_shapes=True,
                    n_streams=3,

...
class EyeTrackingDataImage(EyeTrackingData):
    def __init__(
        ...
        if self.data is None or self.fixations_raw is None:
    ...
    def _compute_calibration_coordinates_trial(self, trial):
        ...
        calibrate_x = calibrate_x * self.x_screen
        calibrate_y = calibrate_y * self.y_screen
    ...
    def _read_csv_trial(self, trial):
        data_trial = None
        for file in os.listdir(self.folder_name + "/" + "vertices"):
            if "_" + str(trial) in file and ".csv" in file and "word_cor_image" in file:
    ...
    def _asign_fixations_words_trial(self, words_fix_trial, fixations_trial):
        ...
        words_fix_trial["fix_duration"] = 0
        words_fix_trial["first_fix_duration"] = 0
        words_fix_trial["fix_number"] = 0
        words_fix_trial["pupil_l"] = 0
        words_fix_trial["pupil_r"] = 0
    ...
    def compute_closets_word_row(
        self,
        words_fix_trial: pd.DataFrame,
        fixation_row: pd.DataFrame,
        word_line_prev,
        fixation_x_prev,
    ):
        """
        ...
        """
        closest_word_row = words_fix_trial.iloc[words_fix_trial["distance"].idxmin()]
        distance = words_fix_trial["distance"].min()
        if (
            closest_word_row["line"] != word_line_prev
            and fixation_row["x"] > fixation_x_prev
        ):
            # we asign this fixation to a word in the same line as the previous fixation
            #if its not more far than 110 of the previous line
            ...
            if abs(fixation_row["y"] - y_mean_line_previous_fix) < 110:
                ...
                closest_word_row = words_fix_trial_new.iloc[
                    words_fix_trial_new["distance"].idxmin()
                ]
                distance = words_fix_trial[words_fix_trial["number"]==closest_word_row["number"]]['distance'].iloc[0]
